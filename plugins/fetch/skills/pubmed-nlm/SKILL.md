...
- **One `<table-wrap>` can hold several `<table>` elements.** PMC splits a table that broke across a page in the original layout, and `find(".//table")` returns only the first. Table 2 of PMID 26041386 is 12 rows plus 10 rows; taking the first block alone yields 9 of its 16 conditions in a table that looks entirely complete — the caption still says *n = 16*. Read every block and concatenate, resolving spans per block since a rowspan cannot cross the break.
